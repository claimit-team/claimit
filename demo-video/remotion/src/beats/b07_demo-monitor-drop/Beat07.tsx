...
export const Beat07: React.FC = () => {
  ...
  const opacity = interpolate(frame, [0, 30], [0, 1], C) * interpolate(frame, [660, 750], [1, 0], { ...C, easing: easings.easeIn });
  ...
  const lastK = n - 1;
  const migT = lastK >= 1 ? interpolate(frame, [DOT_START[lastK], DOT_START[lastK] + MIG], [0, 1], { ...C, easing: ease }) : 1;
  ...
  for (let i = 0; i < n; i++) {
    ...
    dots.push({ xFrac, price, scale: sc, isDrop: i === 3 });
  }

  // Current-price readout (fluctuates as the drop lands, then settles).
  let priceText = "$599.99";
  if (frame >= DROP_START && frame < DROP_START + 38) priceText = CYCLE[Math.min(CYCLE.length - 1, Math.max(0, Math.floor((frame - DROP_START) / 3.2)))];
  ...
  if (frame >= DROP_START + 38 && frame <= DROP_START + 54) priceScale = 1 + 0.05 * Math.sin(((frame - (DROP_START + 38)) / 16) * Math.PI);

  const toastP = interpolate(frame, [DROP_START + 6, DROP_START + 22], [0, 1], { ...C, easing: easings.easeOut });

  return (
    <HookAtmosphere>
      <AbsoluteFill style={{ opacity, transform: `scale(${scale.toFixed(4)})`, transformOrigin: "center center" }}>
        <PriceChartClean dots={dots} checksLogged={n} priceText={priceText} priceScale={priceScale} dropped={dropped} toastP={toastP} />
      </AbsoluteFill>
