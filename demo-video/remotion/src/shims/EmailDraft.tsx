  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: FONT_STACK_TEXT, ...style }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, borderBottom: "1px solid rgba(15,20,25,0.08)", padding: "11px 16px" }}>
        ...
        <span style={{ marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 500, color: colors.text.muted, border: "1px solid rgba(15,20,25,0.12)", borderRadius: 8, padding: "3px 10px" }}>
          {versionLabel}
      ...
      {/* tabs */}
      <div style={{ display: "flex", gap: 20, borderBottom: "1px solid rgba(15,20,25,0.08)", padding: "0 16px" }}>
        {(["Preview", "Edit"] as const).map((t) => {
          const active = t.toLowerCase() === mode;
          return (
            <div key={t} style={{ padding: "9px 2px", fontSize: 13, fontWeight: active ? 600 : 500, color: active ? colors.brand.primary : colors.text.muted, borderBottom: active ? `2px solid ${colors.brand.primary}` : "2px solid transparent" }}>
              {t}
            </div>
          );
        })}
      ...
      <div style={{ flex: 1, overflow: "hidden", padding: 16 }}>
        <div style={{ backgroundColor: "rgba(15,20,25,0.03)", borderRadius: 8, padding: "10px 14px", fontSize: 13, marginBottom: 14 }}>
        ...
        <div
          style={{
            ...
            whiteSpace: "pre-wrap",
            ...(mode === "edit"
              ? { border: "1px solid rgba(39,70,110,0.35)", borderRadius: 8, padding: 12, fontFamily: "ui-monospace, monospace", fontSize: 13 }
              : {}),
          }}
        >
          {shown}
