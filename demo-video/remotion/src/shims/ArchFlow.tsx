...
export const ArchFlow: React.FC<{ litCount: number }> = ({ litCount }) => {
  ...
  return (
    <>
      {/* ---- connector layer ---- */}
      <svg aria-hidden="true" width={1920} height={1080} style={{ position: "absolute", inset: 0 }}>
        <defs>
        ...
        {AGENTS.map((a) => (
          <line
            ...
            strokeWidth={1.5}
            strokeDasharray="5 5"
            opacity={0.3 * vis(a.minStage === 1 ? 3 : a.minStage)}
          />
        ))}
      ...
      <div
        style={{
          ...
          gap: 6,
          opacity: vis(1),
          transform: `scale(${sc(1).toFixed(4)})`,
          transformOrigin: "center",
        }}
      >
        <UploadCloud size={30} color={colors.brand.primary} strokeWidth={2} aria-hidden="true" />
      ...
      {AGENTS.map((a) => (
        ...
        >
          <Img
            src={staticFile("brandlogos/googlegemini.svg")}
      ...
      {["Merchant", "Item", "Date", "Price"].map((f, j) => {
        const t =
          vis(2) *
          interpolate(frame, [40 + j * 8, 64 + j * 8], [0, 1], { ...C, easing: easings.easeOut });
        return (
          <div
            key={f}
            style={{
              ...
              top: ING.y + 70 + j * 34,
              opacity: t,
              transform: `translateY(${((1 - t) * 6).toFixed(1)}px)`,
              fontFamily: FONT_STACK_TEXT,
              fontSize: 12,
              fontWeight: 500,
      ...
      <div
        style={{
          ...
          gap: 8,
          opacity: vis(3),
          transform: `scale(${sc(3).toFixed(4)})`,
          transformOrigin: "center",
        }}
      >
        <svg
      ...
      >
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: colors.text.dark }}>
            MongoDB Atlas
          </div>
          <div style={{ fontSize: 11, color: colors.text.muted, marginTop: 2 }}>
            purchases · claims · policies
          </div>
        </div>
      </div>
      ...
      <div
        style={{
          ...
          padding: 16,
          opacity: vis(5),
          transform: `scale(${sc(5).toFixed(4)})`,
          transformOrigin: "center",
        }}
      >
        ...
        <span
          style={{ fontSize: 11, color: colors.text.muted, textAlign: "center", lineHeight: 1.35 }}
        >
          answers your questions —<br />
          not in the claim workflow
        </span>
      </div>
