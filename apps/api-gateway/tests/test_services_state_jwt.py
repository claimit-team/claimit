...
def test_sign_verify_roundtrip() -> None:
    token = sign_state(user_id="abc-123", return_to="/settings/gmail", key=_KEY)
...
def test_return_to_preserved() -> None:
    token = sign_state("u", "/onboarding/gmail", _KEY)
    payload = verify_state(token, _KEY)
