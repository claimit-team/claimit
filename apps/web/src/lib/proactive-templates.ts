...
function priceDropped(data: PayloadDict): ProactiveOutput {
  ...
  return {
    opening_message: `Your ${platform} item just dropped ${amount}. You have ${hours} hours left in the claim window — want me to file it?`,
    key_facts: [
      `Platform: ${platform}`,
...
function claimDrafted(data: PayloadDict): ProactiveOutput {
  ...
  return {
    opening_message: `I drafted your ${platform} ${claimType} — ${amount} refund. Want me to make it friendlier or explain my reasoning?`,
    key_facts: [`Platform: ${platform}`, `Claim type: ${claimType}`, `Refund amount: ${amount}`],
    quick_actions: [
...
function claimDenied(data: PayloadDict): ProactiveOutput {
  ...
  return {
    ...
    key_facts: [`Platform: ${platform}`, `Denial reason: ${reason}`],
    quick_actions: [
...
function claimResolvedSuccess(data: PayloadDict): ProactiveOutput {
  ...
  let msg = `🎉 You got ${amount} back from ${platform}!`;
